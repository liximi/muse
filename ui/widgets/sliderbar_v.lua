...
local function updateValueInteral(self, value, call_callback, update_block_pos)
	...
	if update_block_pos then
		local new_y = (self.transform.h - self.block.transform.h) * new_val / self.max_limit
		self.block:setPosition(nil, new_y)
	end
end
...
local SliderBar = Class(Widget, function (self, datas, theme)
	...
	self.bg = self:addChild(Panel({
		anchors = {0, 0, 1, 1},
		padding = {0, 0, 0, 0},
		rounding_radius = self.transform.w / 2,
		bg_color = Utils.UI_COLORS.LINE,
		outline_width = 0
	}))

	local block_rounding_radius = (self.transform.w+2)/2
	...
	self.block = self:addChild(Button({
		anchors = {0, 0, 1, 0},
		padding = {-1, -1, 0, nil},
		h = self.block_length_percent * self.transform.h,
...
function SliderBar:setBlockLengthtPercent(percent)
	...
	self.block.transform:setSize(nil, self.block_length_percent * self.transform.h)
end
...
local function moveBlock(self, dir)
	...
	local max_y = self.transform.h - self.block.transform.h
	local delta = self.block.transform.h * self.sensitivity
	local new_y = Utils.clamp(cur_y + (dir == "up" and -delta or delta), 0, max_y)
	if new_y ~= cur_y then
		self.block:setPosition(nil, new_y)
		updateValueInteral(self, self.max_limit * new_y / max_y, true)
	end
end
...
function SliderBar:onMousePressed(x, y, button)
	...
	if is_in_scope then
		...
		if ly <= self.block.transform.y then
			moveBlock(self, "up")
		elseif ly >= self.block.transform.y + self.block.transform.h then
			moveBlock(self, "down")
		end
		return true
	end
...
function SliderBar:onMouseMoved(x, y, dx, dy)
	if self.drag then
		...
		local max_y = self.transform.h - self.block.transform.h
		local new_y = Utils.clamp(cur_y + dy / sy, 0, max_y)
		self.block:setPosition(nil, new_y)
		updateValueInteral(self, self.max_limit * new_y / max_y, true)
	end
end
...
function SliderBar:onSizeChanged(w, h)
	self.block.transform:setSize(nil, self.block_length_percent * h)
