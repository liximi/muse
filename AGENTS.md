...
已覆写 `getMinimumSize` 的控件：Text、Button、Image、ImageButton、ProgressBar、Scroll、BoxContainer、ChatBubble。
...
每次修改代码（API、行为、默认值等），必须同步检查并更新以下文件：
- 源码中的 `--[[datas: ...]]` 注释块
- `docs/` 和 `docs/en/` 下的对应文档
- 本文件（AGENTS.md）中的架构说明和常见陷阱
...
1. **根据代码事实编写**，不得推测或臆断。文档内容必须与当前代码实现一致，包括方法签名、默认值、行为描述。
2. **可读性优先**：文本应简洁、精准、专业。避免黑话和口语化表达。可以使用术语但不宜过多。避免比喻和排比手法。
3. **结构化书写**：使用专业的文档格式（标题层级、表格、代码块），保持与项目现有文档一致的风格。

### 每个控件/接口文档应包含

1. **简要介绍** — 一两句话说明控件的用途和定位。
2. **继承链** — `Widget` → `ButtonBase` → `Button` 形式。
3. **构造参数（datas）** — 用 Lua 代码块列出所有支持的字段，包含类型注释和默认值。
4. **工作原理** — 简要介绍核心机制（尺寸计算、布局算法、自动换行等），不需要深入每个细节。
5. **公有方法表** — 用表格列出主要方法及其说明。
6. **事件处理器表** — 列出可覆写的事件回调（如有）。
7. **示例代码** — 至少一个完整可运行的代码片段。
8. **最佳实践** — 推荐用法和不推荐用法，解释原因，避免用户误解 Muse 的设计意图。

### 语言

- 中文文档放在 `docs/`，英文文档放在 `docs/en/`。
- 两者内容应保持同步，不应出现一个版本有而另一个版本没有的内容。
- 代码块中的注释可使用中文（中文版）或英文（英文版）。

### 同步检查

每次修改代码（API、行为、默认值等）后，必须同步检查并更新：
- 源码中的 `--[[datas: ...]]` 注释块
- `docs/` 和 `docs/en/` 下的对应文档
- `AGENTS.md` 中的架构说明和常见陷阱

### 新增控件

为新增控件编写文档时，还需在以下位置添加引用：
- `docs/` 和 `docs/en/` 目录下新建对应的 `.md` 文件
- 如有总览性质的索引文件，同步更新
