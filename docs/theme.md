...
| 字段 | 默认值 | 说明 |
|------|--------|------|
| `bg_color` | `UI_COLORS.SURFACE` | 背景色 |
...
按钮主题通过 `Utils.newButtonStateStyle()` 定义 6 种状态样式（`normal`/`pressed`/`hover`/`selected`/`selected_hover`/`disabled`）。

状态样式字段：`text`, `text_color`, `font_size`, `bg_color`, `outline_width`, `outline_color`, `offset`, `scale`, `rounding_radius`

> **注意**：按钮文字（`text`）由 `setText()` / 构造参数独立管理，不再参与状态切换合并。`getStateStyle()` 合并时跳过 `text` 字段。

### imagebutton

通过 `Utils.newImageButtonStateStyle()` 定义 6 种状态样式。

状态样式字段：`texture`, `tint`, `text`, `text_color`, `font_size`, `offset`, `scale`
...
| `tab_bg_normal` | `UI_COLORS.BTN_NORMAL` | 未选中背景 |
| `tab_bg_selected` | `UI_COLORS.SURFACE` | 选中背景 |
| `tab_text_normal` | `UI_COLORS.SECONDARY_TEXT` | 未选中文字色 |
| `tab_text_selected` | `UI_COLORS.TITLE` | 选中文字色 |
