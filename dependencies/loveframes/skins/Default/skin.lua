...
function skin.imagebutton(object)
	...
	local width = object:GetWidth()
	local height = object:GetHeight()
	local img_scale_x, img_scale_y = object:GetImgScale()
	local text = object:GetText()
	local hover = object:GetHover()
	...
	if down then
		if image then
			love.graphics.setColor(imagecolor)
			love.graphics.draw(image, x + 1, y + 1, nil, img_scale_x, img_scale_y)
		end
		love.graphics.setFont(font)
	...
	elseif hover then
		if image then
			love.graphics.setColor(imagecolor)
			love.graphics.draw(image, x, y, nil, img_scale_x, img_scale_y)
		end
		love.graphics.setFont(font)
	...
	if checked == true then
		...
		love.graphics.rectangle("line", x+1, y+1, width*img_scale_x-2, height*img_scale_y-2)
	end
