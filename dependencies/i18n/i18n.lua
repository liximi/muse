...
local function gen_get()
    return function(self, key)
        ...
        if lang and type(lang[key]) == "table" and type(lang[key].text) == "string" then
            ...
            return value.text, (fallback == false) and
                (value.audio and string.format("%s/%s", lang.base, value.audio) or false) or false, fallback
        else
            d(string.format("String \"%s\" missing from locale %s and fallback (%s)", key, self.locale, self.fallback))
